...
try {
    ...
    // 1. Check for tickets
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM tickets WHERE user_id IN (2, 11)");
    $stmt->execute();
    $ticketCount = $stmt->fetchColumn();

    // 2. Delete tickets if any (cleanup test data)
    if ($ticketCount > 0) {
        $pdo->query("DELETE FROM tickets WHERE user_id IN (2, 11)");
    }
